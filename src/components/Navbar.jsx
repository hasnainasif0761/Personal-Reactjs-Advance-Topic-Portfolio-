...
function Navbar() {
  ...
  return (
    <nav className={`w-full z-20 h-20 ${showBg ? 'bg-[#070f1c]' : 'bg-transparent'} transition-all duration-300 ease-in-out flex justify-between items-center px-5 fixed top-0 left-0`}>
     <div className='z-10'>
     ...
     {isOpen ? <RxCross1 size={40} onClick={()=>setIsOpen(!isOpen)}  className="flex z-10 md:hidden select-none text-white cursor-pointer" /> : <BsList size={40} onClick={()=>setIsOpen(!isOpen)}  className="flex z-10 md:hidden select-none text-white cursor-pointer" /> }     
    {
        <ul className={`absolute ${showBg ? 'top-full' : 'top-0'} top-0 transition-all duration-300 ease-in-out ${isOpen?'left-0':'-left-full'} w-full bg-black/50 h-screen md:hidden flex flex-col gap-4 justify-center items-center shadow-lg`}>
